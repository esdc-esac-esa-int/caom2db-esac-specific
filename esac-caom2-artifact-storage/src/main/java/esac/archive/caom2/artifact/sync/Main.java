...
public class Main {
    ...
    private static Logger log = Logger.getLogger(esac.archive.caom2.artifact.sync.Main.class);

    public static void main(String[] args) throws Exception {
        ArgumentMap am = new ArgumentMap(args);
        if (am.isSet("d") || am.isSet("debug")) {
            Log4jInit.setLevel("esac.archive.caom2.artifac.sync", Level.DEBUG);
        } else if (am.isSet("v") || am.isSet("verbose")) {
            Log4jInit.setLevel("esac.archive.caom2.artifac.sync", Level.INFO);
        } else {
            Log4jInit.setLevel("esac.archive.caom2.artifac.sync", Level.WARN);
        }
        ...
        ConfigProperties.Init(configFile, collection, forceShutdown);
