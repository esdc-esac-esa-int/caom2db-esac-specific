...
import esac.archive.ehst.dl.caom2.network.performance.db.JdbcSingleton;
...
public class EsacResultsPersistance {
    ...
    private EsacResultsPersistance() {
        try {
            ...
            if (!tableExists) {
            	throw new Exception("Table " + getResultsSchema() + "." + getResultsTable() + " doesn't exist. Check configuration file and permissions in database");
            }
        } catch (Exception e) {
    ...
    public NetworkPerformanceResult select(Date startDate, Date endDate) {
    	if (startDate != null && endDate != null && startDate.after(endDate)) {
    		throw new IllegalArgumentException("Start date must be before end date");    		
    	}
        NetworkPerformanceResult r = new NetworkPerformanceResult();

    	String sDate = null;
    	if (startDate != null) {
    		sDate = new SimpleDateFormat(format).format(startDate);
    		r.setStartDate(startDate);
    	}
    	String eDate = null;
    	if (endDate != null) {
    		eDate = new SimpleDateFormat(format).format(endDate);
        ...
        if (sDate != null && eDate != null) {
        	query = "select * from " + resultsSchema + "." + resultsTable + " where " + getResultsDateColumnName() + " >= '"
                + sDate.toString() + "' and " + getResultsDateColumnName() + " <= '" + eDate + "' order by " + getResultsDateColumnName() + ";";
        } else if (sDate != null) {
        	query = "select * from " + resultsSchema + "." + resultsTable + " where " + getResultsDateColumnName() + " >= '"
                + sDate.toString() + "' order by " + getResultsDateColumnName() + ";";
        } else if (eDate != null) {
        	query = "select * from " + resultsSchema + "." + resultsTable + " where " + getResultsDateColumnName() + " <= '" 
        		+ eDate + "' order by " + getResultsDateColumnName() + ";";
        } else {
        	query = "select * from " + resultsSchema + "." + resultsTable + " order by " + getResultsDateColumnName() + ";";
        }
        ...
        try {
            ...
            long numFileDownloaded = 0;
            long elapsedTimeMs = 0;
            long bytesDownloaded = 0;
            while (rs.next()) {
            	if (rs.isFirst() && startDate == null) {
            		r.setStartDate(new SimpleDateFormat(EsacResultsPersistance.format).parse(rs.getString(getResultsDateColumnName())));
            	}
            	numFileDownloaded += rs.getLong(getResultsSuccessFilesColumnName());
            	elapsedTimeMs += rs.getLong(getResultsElapsedTimeColumnName());
            	bytesDownloaded += rs.getLong(getResultsBytesColumnName());
            	if (rs.isLast() && endDate == null) {
            		r.setEndDate(new SimpleDateFormat(EsacResultsPersistance.format).parse(rs.getString(getResultsDateColumnName())));
            	}
            }
            r.setBytesDownloaded(bytesDownloaded);
            r.setElapsedTimeMs(elapsedTimeMs);
            r.setNumFileDownloaded(numFileDownloaded);
        } catch (Exception ex) {
